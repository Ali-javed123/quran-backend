...
const clearAllQuranCache = async () => {
    try {
        const keys = await redis.keys("*"); // ALL CACHE KEYS
...
export const clearQuranCacheController = async (req, res) => {
    try {
        await clearAllQuranCache();

        return res.json({
            success: true,
            message: "All Quran API cache cleared successfully"
        });
...
//     try {
//         const key = "surahs";  // Cache key change kiya

//         // Redis se cache check
//         let cached = null;
//         try {
...
//         // ✅ FIX: Aggregation with ayahCount (jaise paras me hai)
...
//                     ayahCount: { $sum: 1 }  // ✅ Har surah ki ayat count
//                 }
//             },
//             { $sort: { _id: 1 } },
//             {
...
//                     suraIndex: "$_id",
//                     surah_name: 1,
//                     ayahCount: 1  // ✅ Response me ayahCount bhejna
//                 }
//             }
//         ]);
...
const getSurahs = async (req, res) => {
    try {
        ...
        await setCache(key, surahs); // ✅ array store
...
const getParaByPage = async (req, res) => {
    ...
    const ayahs = await Ayah.find(
        ...
        // 🔥 CRITICAL FIX
        .sort({ globalIndex: 1 })   // ← MOST IMPORTANT
        .lean();
...
const getSurahByPage = async (req, res) => {
    ...
    const ayahs = await Ayah.find(
        ...
        // 🔥 CRITICAL FIX
        .sort({ ayaIndex: 1 })   // ya globalIndex bhi use kar sakte ho
        .lean();
